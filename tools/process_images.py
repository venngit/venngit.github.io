...
def process_image(src: Path, dest_dir: Path, sizes=(800, 400), make_webp=True, quality=85, watermark_text=None):
    results = {}
    try:
        with Image.open(src) as im:
            ...
            for size in sizes:
                ...
                resized.save(dest_path, format='JPEG', quality=quality, optimize=True, progressive=True)
...
def main():
    ...
    parser.add_argument('--sizes', nargs='+', type=int, default=[800, 400], help='Sizes (px) to generate')
    ...
    parser.add_argument('--quality', type=int, default=85, help='JPEG quality for outputs')
    ...
    for f in files:
        ...
        res = process_image(f, dest_dir, sizes=args.sizes, make_webp=args.webp, quality=args.quality, watermark_text=args.watermark)
        mapping[f.name] = res
